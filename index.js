...
const chickenSalad = createMenuItem("Chicken Salad", 4, "Lunch");

const Hamburger = createMenuItem("Hamburger", 3, "Lunch");

const Steak = createMenuItem("Steak", 2, "Dinner");
...
reviews.push({
  name: "Afton",
  rating: 2,
  feedback: "Would never dine again! Almost made me want to puke.",
});
...
reviews[7].feedback =
  "this place is chill with really cool people, great fot getting work done on weekdays";
/* 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 Task 6: 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀
...
*/
function getLastReview(reviewList) {
  return `${reviewList[reviewList.length - 1].name} gave the restaurant a ${
    reviewList[reviewList.length - 1].rating
  }, and their feedback was: ${reviewList[reviewList.length - 1].feedback}`;
}
...
function getReviewByRating(reviewList, starRating) {
  let reviewList2 = [];
  for (let i = 0; i < reviewList.length; i++) {
    if (
      reviewList[i].rating >= starRating &&
      reviewList[i].rating < starRating + 1
    ) {
      reviewList2.push(reviewList[i]);
    }
  }
  return reviewList2;
}
...
*/
function getLongReviews(reviewList) {
  let reviewList2 = [];
  for (let i = 0; i < reviewList.length; i++) {
    if (reviewList[i].feedback.split(" ").length > 15) {
      reviewList2.push(reviewList[i]);
    }
  }
  return reviewList2;
